...
func TestDijkstraWithMinHeap(t *testing.T) {
	tests := []struct {
		name         string
		edges        [][3]int
		_1_indexed   bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			adjList := EdgesToAdjList(tt.vertexCount, tt.edges, tt._1_indexed)

			if gotDistance := DijkstraWithHeap(adjList, tt.vertexCount, tt.start); !reflect.DeepEqual(gotDistance, tt.wantDistance) {
				//gotDistanceBytes, err := json.Marshal(gotDistance)
				//if err != nil {
				//	t.Error(err)
				//}
				...
				t.Errorf("DijkstraWithMinHeap() = %v, want %v", gotDistance, tt.wantDistance)
			}
		})
	}
}
...
func BenchmarkDijkstra(b *testing.B) {
	var res []int
	adjMatrix := EdgesToAdjMatrix(422, bigInput.Edges422, true)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		res = Dijkstra(adjMatrix, 422, 0)
	}
	_ = res
}

func BenchmarkDijkstraWithMinHeap(b *testing.B) {
	var res []int
	adjList := EdgesToAdjList(422, bigInput.Edges422, true)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		res = DijkstraWithHeap(adjList, 422, 0)
	}
	_ = res
}
